<template>
    <div
      id="container"
      class="flex w-full items-center gap-8 border-y-sec border-y  hover:bg-sec transition-all cursor-pointer"
      
    >
      <!-- {{ itemId }} -->
      <div class="flex gap-8 items-center w-full p-4"
      @click="$emit('selectedUser', itemId)">
        ...
        <div class="flex flex-col">
          ...
          <span class="max-w-[6rem] min-w-[6rem] truncate">{{
            object.BDay
          }}</span>
        </div>
      </div>
      <font-awesome-icon
        ...
        class="text-xl ml-8 mr-4 hover:text-accent transition-all"
